...
import statusBarIconView from "./statusBarIconView";
...
export default {
  ...
  consumeStatusBar(statusBar) {
    this.iconView = new statusBarIconView({
      onClick: () => {
        this.start();
      }
    });
  ...
  delayedActivate() {
    ...
    initDnote();
    this.subscriptions.add(
      atom.config.observe("dnote-atom", this.handleConfigChange.bind(this))
    );
  ...
  handleConfigChange(settings) {
    if (settings.showStatusBarIcon && Boolean(this.iconView)) {
      this.iconView.show();
    }
